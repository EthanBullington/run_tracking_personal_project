...
class A_Race(Token_auth):
    ...
    def get(self, request, race_id):
        ...
        race_distance = responseJSON['race']['events'][0]['distance']
        if not race_distance:
            race_distance = "Check Race URL for Distance"
        new_race = Race(distance = race_distance, user = request.user, name = responseJSON['race']['name'], race_id= responseJSON['race']['race_id'], url=responseJSON['race']['url'], next_end_date = next_end_date, city=responseJSON['race']['address']['city'],state=responseJSON['race']['address']['state'], zipcode=responseJSON['race']['address']['zipcode'] )
...
class User_runs(Token_auth):
    ...
    def get(self , request):
        race = Race.objects.filter(user=request.user)
...
class User_run(Token_auth):
    ...
    def get(self, request, race_id):
        race = RaceSerializer(get_object_or_404(Race, race_id = race_id, user = request.user ))
        return Response(race.data, status=HTTP_200_OK)
    

    def delete(self, request, race_id):
        race = get_object_or_404(Race, race_id = race_id, user = request.user )
        race.delete()
